...
// Single-Row-Tabelle (id fix "default"). Ersetzt ClipPilots `streamers`.
export const settings = pgTable("settings", {
  id: text("id").primaryKey().default("default"),
  creatorName: text("creator_name").notNull().default("David"),
...
export const socialAccounts = pgTable("social_accounts", {
  ...
  platform: publishPlatformEnum("platform").notNull().unique(),
  handle: text("handle"),
...
export const sourceVideos = pgTable(
  ...
  (t) => ({
    uqExternal: unique("uq_source_platform_external").on(t.platform, t.externalId),
  })
);
...
export const comments = pgTable(
  ...
  (t) => ({
    uqExternal: unique("uq_comments_platform_external").on(
      t.platform,
      t.externalCommentId
    ),
  })
