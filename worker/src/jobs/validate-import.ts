...
/**
 * ffprobe-Validierung importierter Clips (Davids fertige IG-Clips):
...
export async function processValidateImport(job: Job<ClipJob>): Promise<void> {
  ...
  const localPath = await downloadKeyToTmp(clip.renderedPath, `validate-${clipId}.mp4`);
  try {
    ...
    const aspectOk =
      width > 0 && height > 0 && Math.abs(width / height - TARGET_ASPECT) <= ASPECT_TOLERANCE;
    ...
    await db
      ...
      .set({
        ...
        error: aspectOk
          ? null
          : `Kein 9:16-Format (${width}x${height}) — Blur-Reframe ab Phase 4 möglich`,
        updatedAt: new Date(),
      })
      .where(eq(clips.id, clipId));

    logger.info({ clipId, width, height, duration, aspectOk }, "validate-import: geprüft");
  } catch (err) {
    const message = err instanceof Error ? err.message.slice(0, 500) : String(err);
    await db
      .update(clips)
      .set({ status: "failed", error: `ffprobe fehlgeschlagen: ${message}`, updatedAt: new Date() })
      .where(eq(clips.id, clipId));
    throw err;
  } finally {
