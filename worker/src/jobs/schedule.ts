import { db, clips, posts, settings, socialAccounts } from "@creatorhq/db";
...
function commonTimeSlots(accounts: Array<{ timeSlots: string[] } | undefined>): string[] {
  const lists = accounts.map((account) =>
    account && account.timeSlots.length > 0 ? [...account.timeSlots].sort().join("|") : null
  );
  const first = lists[0];
...
export async function processSchedule(job: Job<ClipJob>): Promise<void> {
  ...
  const missing = targets.filter((platform) => !existingPlatforms.has(platform));
  if (missing.length === 0) {
    await db
      .update(clips)
  ...
  let scheduledAt =
    existing.find((post) => post.scheduledAt !== null && post.scheduledAt > now)
      ?.scheduledAt ?? null;
      ...
      const occupiedRows = await db
        ...
        .where(
          and(
            ...
            gte(posts.scheduledAt, new Date(now.getTime() - OCCUPANCY_WINDOW_MS))
          )
        );
      platforms.push({
        platform,
        clipsPerDay: accountFor(platform)?.clipsPerDay ?? DEFAULT_CLIPS_PER_DAY,
