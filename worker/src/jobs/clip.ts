import { db, sourceVideos, clips } from "@creatorhq/db";
import { classifySource, classificationReason } from "@creatorhq/shared";
...
async function createThumb(
  ...
  try {
    await execa("ffmpeg", [
      "-y", "-ss", String(mid), "-i", localSource,
      "-frames:v", "1", "-vf", "scale=360:-2", "-q:v", "4", local,
    ]);
    const key = `thumbs/${clipId}.jpg`;
    await uploadFile(local, key);
    return key;
  } catch (err) {
    logger.warn({ err, clipId }, "clip: Thumbnail fehlgeschlagen — Kandidat bleibt ohne Bild");
    return null;
  } finally {
...
export async function processClip(job: Job<SourceVideoJob>): Promise<void> {
  ...
  const localSource = await downloadKeyToTmp(video.storagePath, `${sourceVideoId}.mp4`);
  ...
  try {
    ...
    if (env.whisper.enabled && !provider.producesRenderedFile) {
      ...
      for (let i = 0; i < finalCandidates.length; i++) {
        ...
        await execa("ffmpeg", [
          "-y", "-ss", String(candidate.startSeconds), "-to", String(candidate.endSeconds),
          "-i", localSource, "-vn", "-ac", "1", "-ar", "16000", wav,
        ]);
        audioFiles.push(wav);
        idxMap.push(i);
      }
    ...
    for (let i = 0; i < finalCandidates.length; i++) {
      ...
      if (thumbPath) {
        await db.update(clips).set({ thumbPath }).where(eq(clips.id, clip!.id));
      }

      await queues.clip.add("enrich", { clipId: clip!.id });
    }
    ...
    await db
      ...
      .where(eq(sourceVideos.id, sourceVideoId));
    logger.info(
      { sourceVideoId, provider: provider.name, count: finalCandidates.length },
