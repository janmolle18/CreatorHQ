...
export async function processDownload(job: Job<SourceVideoJob>): Promise<void> {
  ...
  try {
    ...
    await execa(env.paths.ytdlp, [
      ...
      "--js-runtimes", "node", // YouTube braucht inzwischen eine JS-Runtime
      "--no-progress",
      "--retries", "10",
      "--fragment-retries", "10",
      ...cookies,
    ...
    const key = `sources/${sourceVideoId}.mp4`;
    ...
    await queues.clip.add("clip", { sourceVideoId }, { jobId: `clip-${sourceVideoId}` });
    logger.info({ sourceVideoId, duration }, "download: fertig, Clipping eingereiht");
  } catch (err) {
