import { clips, sourceVideos } from "@creatorhq/db";
  ...
  await imAuftragsMandanten(job, async (db, tenantId) => {
    ...
    if (clip.origin === "imported" && !reframe) {
      ...
      await db
        ...
        .where(
          and(eq(clips.id, clipId), inArray(clips.status, [...RENDERBAR])),
        );
      logger.info(
        { clipId },
        "render: importierter Clip bleibt im Original — kein Re-Encode",
      );
      return;
    }
    ...
    if (!beansprucht) {
      logger.info(
        { clipId },
        "render: abgebrochen — Clip wurde inzwischen aussortiert",
      );
      return;
    }
    ...
    try {
      ...
      const key = speicherSchluessel(tenantId, "clips", `${clipId}.mp4`);
      ...
      if (!aktualisiert) {
        logger.info(
          { clipId },
          "render: fertig, aber zwischenzeitlich aussortiert — nicht eingeplant",
        );
        return;
      }
      ...
      logger.info(
        { clipId, key },
        "render: 9:16-Clip fertig, Scheduling eingereiht",
      );
    } catch (err) {
      const message =
